...
接下来的n行，每行输入两个整数a_i,b_i(0≤ai<bi≤10^9)，表示第i个活动的时间。
输出描述：
...
描述
给出一个有n种字符组成的字符串，其中第ii种字符出现的次数为a_i。请你对该字符串应用哈夫曼编码，使得该字符串的长度尽可能短，求编码后的字符串的最短长度。
输入描述：
第一行输入一个整数n，表示字符种数。
第二行输入n个整数，表示每种字符的出现次数。
输出描述：
...
数据范围：三角形数组行数满足 1≤n≤200  ，数组中的值都满足 ∣val∣≤10^4

例如当输入[[2],[3,4],[6,5,7],[4,1,8,3]]时，对应的输出为11，
所选的路径如下图所示：

![546EEFF3F6EF9BB6284C4EEAD52F618E](/Users/zzy/Downloads/Chrome下载/546EEFF3F6EF9BB6284C4EEAD52F618E.png)
...
第二行为 n 个整数 a_i，用空格隔开，代表数组中的每一个数。 ∣a i∣≤10^2
...
接下来n行，每行两个数v_i和w_i，表示第i个物品的体积和价值。
...
输入描述：
第一行两个整数n和V，表示物品个数和背包体积。
接下来n行，每行两个数v_i和w_i，表示第i种物品的体积和价值。
1 ≤n,V≤1000
